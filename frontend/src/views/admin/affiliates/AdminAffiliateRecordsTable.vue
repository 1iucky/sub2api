      ...
      <template #table>
        ...
        >
          ...
          <template #cell-aff_code="{ row }">
            <span class="font-mono text-sm tabular-nums text-gray-700 dark:text-gray-300">{{ row.aff_code || '-' }}</span>
          </template>
          <template #cell-order="{ row }">
            <div class="space-y-0.5">
              <div class="font-mono text-sm tabular-nums text-gray-900 dark:text-white">#{{ row.order_id }}</div>
              <div class="max-w-56 truncate font-mono text-xs text-gray-500 dark:text-dark-400">{{ row.out_trade_no }}</div>
            </div>
          </template>
          ...
          <template #cell-pay_amount="{ row }">
            <span class="text-sm tabular-nums text-gray-900 dark:text-white">¥{{ formatAmount(row.pay_amount) }}</span>
          </template>
          ...
          <template #cell-created_at="{ row }">
            <span class="font-mono text-xs tabular-nums text-gray-700 dark:text-gray-300">{{ formatDateTime(row.created_at) }}</span>
          </template>
        </DataTable>
      </template>
    ...
    >
      ...
      <div v-else-if="selectedOverview" class="space-y-4">
        <div class="rounded-md border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
          <div class="font-mono text-sm tabular-nums text-gray-900 dark:text-white">#{{ selectedOverview.user_id }}</div>
          <div class="mt-1 text-sm font-normal text-gray-900 dark:text-white">{{ selectedOverview.email || '-' }}</div>
...
const UserCell = defineComponent({
  ...
  setup(cellProps, { emit }) {
    return () => h('div', { class: 'space-y-0.5' }, [
      h('div', { class: 'font-mono text-sm tabular-nums text-gray-900 dark:text-white' }, `#${cellProps.id}`),
      h(cellProps.clickable ? 'button' : 'div', {
        class: cellProps.clickable
          ? 'max-w-56 truncate text-left text-sm font-normal text-primary-600 transition-colors duration-150 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300'
      ...
      h('div', { class: 'max-w-56 truncate text-xs text-gray-500 dark:text-dark-400' }, cellProps.username || '-'),
    ])
  },
})
  ...
  setup(amountProps) {
    return () => h('span', {
      class: amountProps.strong
        ? 'text-sm font-normal tabular-nums text-emerald-600 dark:text-emerald-400'
        : 'text-sm font-normal tabular-nums text-gray-900 dark:text-white',
...
const OverviewStat = defineComponent({
  ...
  setup(statProps) {
    return () => h('div', { class: 'rounded-md border border-gray-200 bg-white p-3 dark:border-dark-700 dark:bg-dark-900' }, [
      h('div', { class: 'eyebrow' }, statProps.label),
      h('div', {
        class: statProps.mono
          ? 'mt-1.5 font-mono text-base font-normal tabular-nums text-gray-900 dark:text-white'
          : 'mt-1.5 text-base font-normal tabular-nums text-gray-900 dark:text-white',
      }, statProps.value),
    ])
  },
})
