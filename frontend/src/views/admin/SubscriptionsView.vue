          ...
          <div class="flex flex-1 flex-wrap items-center gap-3">
            ...
            >
              ...
              <div
                ...
                class="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-sm border border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-900"
              >
                <div
                ...
                <button
                  ...
                  class="w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-dark-800"
                >
                  <span class="font-medium text-gray-900 dark:text-white">{{ user.email }}</span>
                  <span class="ml-2 font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">#{{ user.id }}</span>
                </button>
              </div>
            </div>
          ...
          <div class="ml-auto flex flex-wrap items-center justify-end gap-3">
            ...
            <div class="relative" ref="columnDropdownRef">
              ...
              <div
                ...
                class="absolute right-0 z-50 mt-2 w-48 origin-top-right rounded-sm border border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-900"
              >
                <div class="p-2">
                  <!-- User column mode selection -->
                  <div class="mb-2 border-b border-dashed border-gray-200 pb-2 dark:border-dark-700">
                    <div class="eyebrow px-3 py-1">
                      {{ t('admin.subscriptions.columns.user') }}
                    </div>
                    <button
                      @click="setUserColumnMode('email')"
                      class="flex w-full items-center justify-between rounded-sm px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-dark-800"
                    >
        ...
        >
            ...
            <div class="min-w-[280px] space-y-2">
              ...
              <div v-if="row.group?.daily_limit_usd" class="usage-row">
                <div class="flex items-center gap-2">
                  ...
                  <div class="h-1.5 flex-1 rounded-full bg-gray-200 dark:bg-dark-700">
                    <div
                      class="h-1.5 rounded-full transition-colors duration-150"
              ...
              <div
                ...
                "
                class="inline-flex items-center gap-2 rounded-sm border border-dashed border-emerald-300 bg-emerald-50 px-3 py-2 dark:border-emerald-900/50 dark:bg-emerald-900/10"
              >
                <span class="font-mono text-lg text-emerald-600 dark:text-emerald-400">∞</span>
                <span class="font-mono text-xs uppercase tracking-wide text-emerald-700 dark:text-emerald-300">
          ...
          <template #cell-expires_at="{ value }">
            <div v-if="value">
              ...
              <div v-if="getDaysRemaining(value) !== null" class="eyebrow mt-0.5">
                {{ getDaysRemaining(value) }} {{ t('admin.subscriptions.daysRemaining') }}
              </div>
            </div>
            <span v-else class="font-mono text-xs uppercase tracking-wide text-gray-500 dark:text-dark-400">{{
          ...
          <template #cell-status="{ value }">
            <span
              :class="[
                'badge font-mono uppercase tracking-wide',
                value === 'active'
                  ? 'badge-success'
                  : value === 'expired'
                    ? 'badge-warning'
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-sm p-1.5 text-gray-500 transition-colors hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-900/20 dark:hover:text-blue-400"
              >
                <Icon name="calendar" size="sm" />
                <span class="font-mono text-[10px] uppercase tracking-wide">{{ t('admin.subscriptions.adjust') }}</span>
              </button>
              <button
                v-if="row.status === 'active'"
                @click="handleResetQuota(row)"
                :disabled="resettingQuota && resettingSubscription?.id === row.id"
                class="flex flex-col items-center gap-0.5 rounded-sm p-1.5 text-gray-500 transition-colors hover:bg-amber-50 hover:text-amber-600 dark:hover:bg-amber-900/20 dark:hover:text-amber-400 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <Icon name="refresh" size="sm" />
                <span class="font-mono text-[10px] uppercase tracking-wide">{{ t('admin.subscriptions.resetQuota') }}</span>
              </button>
              <button
                v-if="row.status === 'active'"
              ...
              >
                ...
                <span class="font-mono text-[10px] uppercase tracking-wide">{{ t('admin.subscriptions.revoke') }}</span>
              </button>
              <button
    ...
    <teleport to="body">
      <transition name="modal">
        <div v-if="showGuideModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="showGuideModal = false">
          ...
          <div class="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-lg border border-gray-200 bg-white p-6 dark:border-dark-700 dark:bg-dark-900">
            ...
            <div class="eyebrow mb-2">{{ t('admin.subscriptions.guide.subtitle') }}</div>
            <h2 class="mb-5 font-normal text-2xl tracking-tight text-gray-900 dark:text-white">{{ t('admin.subscriptions.guide.title') }}</h2>

            <!-- Step 1 -->
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 font-normal text-base tracking-tight text-gray-900 dark:text-white">
                <span class="flex h-6 w-6 items-center justify-center rounded-sm border border-primary-200 bg-primary-50 font-mono text-xs font-medium text-primary-600 dark:border-primary-900/40 dark:bg-primary-900/20 dark:text-primary-400">1</span>
              ...
              <div class="ml-8 mt-2">
                <router-link
                  ...
                  class="inline-flex items-center gap-1 font-mono text-xs uppercase tracking-wide text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300"
                >
            ...
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 font-normal text-base tracking-tight text-gray-900 dark:text-white">
                <span class="flex h-6 w-6 items-center justify-center rounded-sm border border-primary-200 bg-primary-50 font-mono text-xs font-medium text-primary-600 dark:border-primary-900/40 dark:bg-primary-900/20 dark:text-primary-400">2</span>
            ...
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 font-normal text-base tracking-tight text-gray-900 dark:text-white">
                <span class="flex h-6 w-6 items-center justify-center rounded-sm border border-primary-200 bg-primary-50 font-mono text-xs font-medium text-primary-600 dark:border-primary-900/40 dark:bg-primary-900/20 dark:text-primary-400">3</span>
                {{ t('admin.subscriptions.guide.step3.title') }}
              </h3>
              <div class="ml-8 overflow-hidden rounded-md border border-gray-200 dark:border-dark-700">
                <table class="w-full text-sm">
                  <tbody>
                    <tr v-for="(row, i) in guideActionRows" :key="i" class="border-b border-dashed border-gray-200 last:border-0 dark:border-dark-700">
                      <td class="whitespace-nowrap bg-gray-50 px-3 py-2 font-mono text-xs uppercase tracking-wide text-gray-700 dark:bg-dark-800 dark:text-gray-300">{{ row.action }}</td>
            ...
            <!-- Tip -->
            <div class="rounded-md border border-dashed border-blue-300 bg-blue-50 p-3 text-xs text-blue-700 dark:border-blue-900/40 dark:bg-blue-900/10 dark:text-blue-300">
...
.usage-label {
  @apply w-10 flex-shrink-0 font-mono text-[10px] font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400;
}

.usage-amount {
  @apply whitespace-nowrap font-mono text-xs tabular-nums text-gray-600 dark:text-gray-300;
}

.reset-info {
  @apply flex items-center gap-1 pl-12 font-mono text-[10px] uppercase tracking-wide text-blue-600 dark:text-blue-400;
}
</style>
