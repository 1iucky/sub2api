...
<template>
  <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-dark-700 dark:bg-dark-900">
    <div class="mb-4 flex items-start justify-between gap-4 border-b border-dashed border-gray-200 pb-4 dark:border-dark-700">
      <div>
        <div class="eyebrow">{{ t('admin.ops.runtime.title') }}</div>
    ...
    <div v-else class="space-y-6">
      <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60">
        <div class="mb-3 flex items-center justify-between">
          <h4 class="text-sm font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.ops.runtime.alertTitle') }}</h4>
          <button class="btn btn-sm btn-secondary" @click="openAlertEditor">{{ t('common.edit') }}</button>
        </div>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="font-mono text-xs tabular-nums text-gray-600 dark:text-gray-300">
          ...
          <div
            ...
            class="font-mono text-xs tabular-nums text-gray-600 dark:text-gray-300 md:col-span-2"
          >
            {{ t('admin.ops.runtime.silencing.globalUntil') }}:
            <span class="ml-1 text-gray-900 dark:text-white">{{ alertSettings.silencing.global_until_rfc3339 }}</span>
          </div>

          <details class="col-span-1 md:col-span-2">
            <summary class="cursor-pointer font-mono text-xs font-medium text-primary-600 hover:text-primary-700 dark:text-primary-400">
              {{ t('admin.ops.runtime.showAdvancedDeveloperSettings') }}
            </summary>
            <div class="mt-2 grid grid-cols-1 gap-3 rounded-sm border border-gray-200 bg-white p-3 dark:border-dark-700 dark:bg-dark-900 md:grid-cols-2">
              <div class="font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">
                {{ t('admin.ops.runtime.lockEnabled') }}:
                <span class="ml-1 text-gray-700 dark:text-gray-300">{{ alertSettings.distributed_lock.enabled }}</span>
              </div>
              <div class="font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">
                {{ t('admin.ops.runtime.lockKey') }}:
                <span class="ml-1 text-gray-700 dark:text-gray-300">{{ alertSettings.distributed_lock.key }}</span>
              </div>
              <div class="font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">
                {{ t('admin.ops.runtime.lockTTLSeconds') }}:
                <span class="ml-1 text-gray-700 dark:text-gray-300">{{ alertSettings.distributed_lock.ttl_seconds }}s</span>
              </div>
            </div>
          </details>
        </div>
  ...
  <BaseDialog :show="showAlertEditor" :title="t('admin.ops.runtime.alertTitle')" width="extra-wide" @close="showAlertEditor = false">
    <div v-if="draftAlert" class="space-y-4">
      <div
        ...
        class="rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800 dark:border-amber-900/50 dark:bg-amber-900/20 dark:text-amber-200"
      >
        <div class="font-mono font-medium">{{ t('admin.ops.runtime.validation.title') }}</div>
      ...
      <div>
        <div class="mb-1 font-mono text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.runtime.evalIntervalSeconds') }}</div>
        <input
      ...
      <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60">
        <div class="mb-2 text-sm font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.ops.runtime.metricThresholds') }}</div>
        <p class="mb-4 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.ops.runtime.metricThresholdsHint') }}</p>

        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 font-mono text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.runtime.slaMinPercent') }}</div>
            <input
          ...
          <div>
            <div class="mb-1 font-mono text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.runtime.ttftP99MaxMs') }}</div>
            <input
          ...
          <div>
            <div class="mb-1 font-mono text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.runtime.requestErrorRateMaxPercent') }}</div>
            <input
          ...
          <div>
            <div class="mb-1 font-mono text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.runtime.upstreamErrorRateMaxPercent') }}</div>
            <input
      ...
      <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60">
        <div class="mb-2 text-sm font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.ops.runtime.silencing.title') }}</div>
        ...
        <div v-if="draftAlert.silencing.enabled" class="mt-4 space-y-4">
          <div>
            <div class="mb-1 font-mono text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.runtime.silencing.globalUntil') }}</div>
            <input
          ...
          <div>
            <div class="mb-1 font-mono text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.runtime.silencing.reason') }}</div>
            <input
          ...
          <div class="rounded-md border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
            <div class="flex items-start justify-between gap-4 border-b border-dashed border-gray-200 pb-3 dark:border-dark-700">
              <div>
                <div class="font-mono text-xs font-medium text-gray-900 dark:text-white">{{ t('admin.ops.runtime.silencing.entries.title') }}</div>
                <p class="font-mono text-[11px] text-gray-500 dark:text-gray-400">{{ t('admin.ops.runtime.silencing.entries.hint') }}</p>
              </div>
            ...
            <div v-if="!draftAlert.silencing.entries?.length" class="mt-3 rounded-sm border border-dashed border-gray-200 bg-gray-50 p-3 text-xs text-gray-500 dark:border-dark-700 dark:bg-dark-800/60 dark:text-gray-400">
            ...
            <div v-else class="mt-4 space-y-4">
              <div
                ...
                :key="idx"
                class="rounded-sm border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60"
              >
                <div class="mb-3 flex items-center justify-between">
                ...
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <div>
                    <div class="mb-1 font-mono text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.runtime.silencing.entries.ruleId') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 font-mono text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.runtime.silencing.entries.severities') }}</div>
                    <input
                  ...
                  <div>
                    <div class="mb-1 font-mono text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.runtime.silencing.entries.until') }}</div>
                    <input
              ...
              placeholder="2026-01-05T00:00:00Z"
                  ...
                  <div>
                    <div class="mb-1 font-mono text-xs font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.runtime.silencing.entries.reason') }}</div>
                    <input
      ...
      <details class="rounded-md border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800/60">
        <summary class="cursor-pointer font-mono text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('admin.ops.runtime.advancedSettingsSummary') }}</summary>
        <div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            <div class="mb-1 font-mono text-xs font-medium text-gray-500">{{ t('admin.ops.runtime.lockKey') }}</div>
          ...
          <div>
            <div class="mb-1 font-mono text-xs font-medium text-gray-500">{{ t('admin.ops.runtime.lockTTLSeconds') }}</div>
