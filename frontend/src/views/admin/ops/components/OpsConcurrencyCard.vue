...
function getLoadBarClass(loadPct: number): string {
  if (loadPct >= 90) return 'bg-red-500 dark:bg-red-500'
  if (loadPct >= 70) return 'bg-amber-500 dark:bg-amber-500'
  if (loadPct >= 50) return 'bg-primary-500 dark:bg-primary-500'
...
function getLoadTextClass(loadPct: number): string {
  ...
  if (loadPct >= 70) return 'text-amber-600 dark:text-amber-400'
...
<template>
  <div class="flex h-full flex-col rounded-lg border border-gray-200 bg-white p-6 dark:border-dark-700 dark:bg-dark-900">
    <!-- 头部 -->
    <div class="mb-4 flex shrink-0 items-center justify-between gap-3 border-b border-dashed border-gray-200 pb-3 dark:border-dark-700">
      ...
      <div class="flex items-center gap-2">
        <!-- 用户视图切换按钮 -->
        <button
          class="flex items-center justify-center rounded-sm border px-2 py-1 transition-colors"
          :class="showByUser
            ...
            : 'border-gray-200 bg-white text-gray-500 hover:bg-gray-100 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-400 dark:hover:bg-dark-700'"
        ...
        <!-- 刷新按钮 -->
        <button
          class="flex items-center gap-1 rounded-sm border border-gray-200 bg-white px-2 py-1 font-mono text-[11px] font-medium text-gray-700 transition-colors hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-300 dark:hover:bg-dark-700"
          :disabled="loading"
    ...
    <!-- 错误提示 -->
    <div v-if="errorMessage" class="mb-3 shrink-0 rounded-sm border border-red-200 bg-red-50 p-2.5 text-xs text-red-600 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-400">
      {{ errorMessage }}
    </div>
    ...
    <div
      ...
      class="flex flex-1 items-center justify-center rounded-md border border-dashed border-gray-200 text-sm text-gray-500 dark:border-dark-700 dark:text-gray-400"
    >
      {{ t('admin.ops.concurrency.disabledHint') }}
    </div>

    <!-- 数据展示区域 -->
    <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-md border border-gray-200 dark:border-dark-700">
      <!-- 维度标题栏 -->
      <div class="flex shrink-0 items-center justify-between border-b border-dashed border-gray-200 bg-gray-50 px-3 py-2 dark:border-dark-700 dark:bg-dark-900">
        <span class="eyebrow">
          {{ displayTitle }}
        </span>
      ...
      <div v-else-if="displayDimension === 'user'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as UserRow[])" :key="row.key" class="rounded-sm border border-gray-200 bg-gray-50 p-2.5 dark:border-dark-700 dark:bg-dark-800/60">
          <!-- 用户信息和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="flex min-w-0 flex-1 items-center gap-1.5">
              <span class="truncate font-mono text-[11px] font-medium text-gray-900 dark:text-white" :title="row.username || row.user_email">
                {{ row.username || row.user_email }}
              </span>
              <span v-if="row.username" class="shrink-0 truncate font-mono text-[10px] text-gray-400 dark:text-gray-500" :title="row.user_email">
                {{ row.user_email }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 font-mono text-[10px] tabular-nums">
              <span class="font-medium text-gray-900 dark:text-white"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
              <span :class="['font-medium', getLoadTextClass(row.load_percentage)]"> {{ Math.round(row.load_percentage) }}% </span>
            </div>
          </div>

          <!-- 进度条 -->
          <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-700">
          ...
          <div v-if="row.waiting_in_queue > 0" class="mt-1.5 flex justify-end">
            <span class="rounded-sm bg-primary-100 px-1.5 py-0.5 font-mono text-[10px] font-medium text-primary-700 dark:bg-primary-500/15 dark:text-primary-400">
      ...
      <div v-else-if="displayDimension === 'platform' || displayDimension === 'group'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as SummaryRow[])" :key="row.key" class="rounded-sm border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800/60">
          <!-- 标题行 -->
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              <div class="truncate font-mono text-[11px] font-medium text-gray-900 dark:text-white" :title="row.name">
                {{ row.name }}
              </div>
              <span v-if="displayDimension === 'group' && row.platform" class="font-mono text-[10px] uppercase tracking-wide text-gray-400 dark:text-gray-500">
                {{ row.platform.toUpperCase() }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 font-mono text-[10px] tabular-nums">
              <span class="font-medium text-gray-900 dark:text-white"> {{ row.used_concurrency }}/{{ row.total_concurrency }} </span>
              <span :class="['font-medium', getLoadTextClass(row.concurrency_percentage)]"> {{ row.concurrency_percentage }}% </span>
            </div>
          </div>

          <!-- 进度条 -->
          <div class="mb-2 h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-700">
            <div
          ...
          <!-- 统计信息 -->
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[10px] tabular-nums">
            <!-- 账号统计 -->
            <div class="flex items-center gap-1">
              ...
              <span class="text-gray-600 dark:text-gray-300">
                <span class="font-medium text-emerald-600 dark:text-emerald-400">{{ row.available_accounts }}</span
                >/{{ row.total_accounts }}
              </span>
            ...
            <span
              ...
              class="rounded-sm bg-amber-100 px-1.5 py-0.5 font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
            >
            ...
            <span
              ...
              class="rounded-sm bg-red-100 px-1.5 py-0.5 font-medium text-red-700 dark:bg-red-900/30 dark:text-red-400"
            >
            ...
            <span
              ...
              class="rounded-sm bg-primary-100 px-1.5 py-0.5 font-medium text-primary-700 dark:bg-primary-500/15 dark:text-primary-400"
            >
              {{ t('admin.ops.concurrency.queued', { count: row.waiting_in_queue }) }}
            </span>
          </div>
      ...
      <div v-else class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as AccountRow[])" :key="row.key" class="rounded-sm border border-gray-200 bg-gray-50 p-2.5 dark:border-dark-700 dark:bg-dark-800/60">
          <!-- 账号名称和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-0.5 font-mono text-[9px] text-gray-400 dark:text-gray-500">
                {{ row.group_name }}
              </div>
            </div>
            <div class="flex shrink-0 items-center gap-2">
              <!-- 并发使用 -->
              <span class="font-mono text-[11px] font-medium tabular-nums text-gray-900 dark:text-white"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
              <!-- 状态徽章 -->
              <span
                v-if="row.is_available"
                class="inline-flex items-center gap-1 rounded-sm bg-emerald-100 px-1.5 py-0.5 font-mono text-[10px] font-medium text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400"
              >
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded-sm bg-amber-100 px-1.5 py-0.5 font-mono text-[10px] font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
              >
                <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded-sm bg-red-100 px-1.5 py-0.5 font-mono text-[10px] font-medium text-red-700 dark:bg-red-900/30 dark:text-red-400"
              >
                <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path
                    stroke-linecap="round"
              ...
              <span
                v-else
                class="inline-flex items-center gap-1 rounded-sm bg-gray-100 px-1.5 py-0.5 font-mono text-[10px] font-medium text-gray-700 dark:bg-dark-800 dark:text-gray-400"
              >
...
.custom-scrollbar {
  ...
  scrollbar-color: rgba(160, 148, 134, 0.3) transparent;
}
...
.custom-scrollbar::-webkit-scrollbar-thumb {
  background-color: rgba(160, 148, 134, 0.3);
  border-radius: 3px;
}

.custom-scrollbar::-webkit-scrollbar-thumb:hover {
  background-color: rgba(160, 148, 134, 0.5);
}
</style>
