...
function statusBadgeClass(status: string | undefined): string {
  ...
  if (s === 'resolved') return 'bg-emerald-50 text-emerald-700 ring-emerald-600/20 dark:bg-emerald-900/30 dark:text-emerald-300 dark:ring-emerald-500/30'
  if (s === 'manual_resolved') return 'bg-gray-50 text-gray-700 ring-gray-600/20 dark:bg-dark-700 dark:text-gray-300 dark:ring-dark-500/30'
  return 'bg-gray-50 text-gray-700 ring-gray-600/20 dark:bg-dark-800 dark:text-gray-300 dark:ring-dark-500/30'
}
...
<template>
  <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-dark-700 dark:bg-dark-900">
    <div class="mb-4 flex flex-col gap-3 border-b border-dashed border-gray-200 pb-4 sm:flex-row sm:items-start sm:justify-between sm:gap-4 dark:border-dark-700">
      <div>
        <div class="eyebrow">{{ t('admin.ops.alertEvents.title') }}</div>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <button
          class="flex items-center gap-1.5 rounded-sm border border-gray-200 bg-white px-3 py-1.5 font-mono text-xs font-medium text-gray-700 transition-colors hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-300 dark:hover:bg-dark-700"
          :disabled="loading"
          @click="loadFirstPage"
        >
    ...
    <div v-else-if="empty" class="rounded-md border border-dashed border-gray-200 p-8 text-center text-sm text-gray-500 dark:border-dark-700 dark:text-gray-400">
    ...
    <div v-else class="overflow-hidden rounded-md border border-gray-200 dark:border-dark-700">
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        ...
        <table v-else class="min-w-full divide-y divide-dashed divide-gray-200 dark:divide-dark-700">
          <thead class="sticky top-0 z-10 bg-gray-50 dark:bg-dark-900">
            <tr>
              <th class="eyebrow px-4 py-3 text-left">
              ...
              <th class="eyebrow px-4 py-3 text-right">
                {{ t('admin.ops.alertEvents.table.email') }}
              </th>
            </tr>
          </thead>
          <tbody class="divide-y divide-dashed divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
            <tr
              v-for="row in events"
              :key="row.id"
              ...
              :title="row.title || ''"
            >
              <td class="whitespace-nowrap px-4 py-3 font-mono text-xs tabular-nums text-gray-600 dark:text-gray-300">
                {{ formatDateTime(row.fired_at || row.created_at) }}
              </td>
              <td class="whitespace-nowrap px-4 py-3">
                <div class="flex items-center gap-2">
                  <span class="rounded-sm px-2 py-1 font-mono text-[10px] font-medium" :class="severityBadgeClass(String(row.severity || ''))">
                    {{ row.severity || '-' }}
                  </span>
                  <span class="inline-flex items-center rounded-sm px-2 py-1 font-mono text-[10px] font-medium ring-1 ring-inset" :class="statusBadgeClass(row.status)">
                    {{ formatStatusLabel(row.status) }}
                  </span>
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 font-mono text-xs text-gray-600 dark:text-gray-300">
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-gray-700 dark:text-gray-200">
                <div class="font-medium truncate max-w-[360px]">{{ row.title || '-' }}</div>
              ...
              <td class="whitespace-nowrap px-4 py-3 font-mono text-[11px] tabular-nums text-gray-500 dark:text-gray-400">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-right text-xs">
                ...
                >
                  ...
                  <Icon
                    ...
                    class="text-gray-400 dark:text-gray-500"
                  />
                  <span class="font-mono text-[11px] font-medium text-gray-600 dark:text-gray-300">
                    {{ row.email_sent ? t('admin.ops.alertEvents.table.emailSent') : t('admin.ops.alertEvents.table.emailIgnored') }}
                  </span>
                </span>
              </td>
      ...
      <div v-else class="space-y-5">
        <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div class="flex flex-wrap items-center gap-2">
                <span class="inline-flex items-center rounded-sm px-2 py-1 font-mono text-[10px] font-medium" :class="severityBadgeClass(String(selected.severity || ''))">
                  {{ selected.severity || '-' }}
                </span>
                <span class="inline-flex items-center rounded-sm px-2 py-1 font-mono text-[10px] font-medium ring-1 ring-inset" :class="statusBadgeClass(selected.status)">
              ...
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap font-mono text-xs text-gray-600 dark:text-gray-300">
                {{ selected.description }}
              </div>
            </div>

            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-sm border border-gray-200 bg-white px-2 py-1 dark:border-dark-700 dark:bg-dark-900">
                <span class="font-mono text-[11px] font-medium text-gray-600 dark:text-gray-300">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60">
              <div class="eyebrow">{{ t('admin.ops.alertEvents.detail.firedAt') }}</div>
              <div class="mt-1 font-mono text-sm tabular-nums text-gray-900 dark:text-white">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60">
              <div class="eyebrow">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 font-mono text-sm tabular-nums text-gray-900 dark:text-white">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60">
              <div class="eyebrow">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-medium tabular-nums text-gray-900 dark:text-white">#{{ selected.rule_id }}</div>
                <a
                  class="inline-flex items-center gap-1 rounded-sm border border-gray-200 bg-white px-2 py-1 font-mono text-[11px] font-medium text-gray-700 hover:bg-gray-50 dark:border-dark-700 dark:bg-dark-900 dark:text-gray-200 dark:hover:bg-dark-800"
            ...
            <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60">
              <div class="eyebrow">{{ t('admin.ops.alertEvents.detail.dimensions') }}</div>
        ...
        <div class="rounded-md border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3 border-b border-dashed border-gray-200 pb-3 dark:border-dark-700">
            <div>
              <div class="text-sm font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.ops.alertEvents.detail.historyTitle') }}</div>
          ...
          <div v-else class="overflow-hidden rounded-sm border border-gray-200 dark:border-dark-700">
            <table class="min-w-full divide-y divide-dashed divide-gray-200 dark:divide-dark-700">
              <thead class="bg-gray-50 dark:bg-dark-900">
                <tr>
                  <th class="eyebrow px-3 py-2 text-left">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="eyebrow px-3 py-2 text-left">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="eyebrow px-3 py-2 text-left">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-dashed divide-gray-200 dark:divide-dark-700">
                <tr v-for="it in history" :key="it.id" class="hover:bg-gray-50 dark:hover:bg-dark-800/60">
                  <td class="px-3 py-2 font-mono text-xs tabular-nums text-gray-600 dark:text-gray-300">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
                  <td class="px-3 py-2 text-xs">
                    <span class="inline-flex items-center rounded-sm px-2 py-1 font-mono text-[10px] font-medium ring-1 ring-inset" :class="statusBadgeClass(it.status)">
                      {{ formatStatusLabel(it.status) }}
                    </span>
                  </td>
                  <td class="px-3 py-2 font-mono text-xs tabular-nums text-gray-600 dark:text-gray-300">
