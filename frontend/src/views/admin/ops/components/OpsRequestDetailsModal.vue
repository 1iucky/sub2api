...
const kindBadgeClass = (kind: string) => {
  if (kind === 'error') return 'bg-red-50 text-red-700 ring-red-600/20 dark:bg-red-900/30 dark:text-red-300 dark:ring-red-500/30'
  return 'bg-emerald-50 text-emerald-700 ring-emerald-600/20 dark:bg-emerald-900/30 dark:text-emerald-300 dark:ring-emerald-500/30'
}
</script>
      ...
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div v-if="loading" class="flex flex-1 items-center justify-center py-16">
          <div class="flex flex-col items-center gap-3">
            ...
            <span class="font-mono text-sm text-gray-500 dark:text-gray-400">{{ t('common.loading') }}</span>
          </div>
        </div>

        <!-- Table -->
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div v-if="items.length === 0" class="rounded-md border border-dashed border-gray-200 p-10 text-center dark:border-dark-700">
            <div class="text-sm text-gray-600 dark:text-gray-300">{{ t('admin.ops.requestDetails.empty') }}</div>
          ...
          <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-md border border-gray-200 dark:border-dark-700">
            <div class="min-h-0 flex-1 overflow-auto">
              ...
              <table v-else class="min-w-full divide-y divide-dashed divide-gray-200 dark:divide-dark-700">
                ...
                <tr>
                  <th class="border-b border-gray-200 px-4 py-3 text-left font-mono text-[11px] font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">
                  ...
                  <th class="border-b border-gray-200 px-4 py-3 text-right font-mono text-[11px] font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">
              ...
              <tbody class="divide-y divide-dashed divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
                <tr v-for="(row, idx) in items" :key="idx" class="transition-colors hover:bg-gray-50 dark:hover:bg-dark-800/60">
                  <td class="whitespace-nowrap px-4 py-3 font-mono text-[11px] tabular-nums text-gray-600 dark:text-gray-300">
                    {{ formatDateTime(row.created_at) }}
                  </td>
                  <td class="whitespace-nowrap px-4 py-3">
                    <span class="rounded-sm px-2 py-1 font-mono text-[10px] font-medium ring-1 ring-inset" :class="kindBadgeClass(row.kind)">
                      {{ row.kind === 'error' ? t('admin.ops.requestDetails.kind.error') : t('admin.ops.requestDetails.kind.success') }}
                    </span>
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 font-mono text-[11px] font-medium uppercase text-gray-700 dark:text-gray-200">
                    {{ (row.platform || 'unknown').toUpperCase() }}
                  </td>
                  <td class="max-w-[240px] truncate px-4 py-3 font-mono text-[11px] text-gray-600 dark:text-gray-300" :title="row.model || ''">
                    {{ row.model || '-' }}
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 font-mono text-[11px] tabular-nums text-gray-600 dark:text-gray-300">
                  ...
                  <td class="px-4 py-3">
                    <div v-if="row.request_id" class="flex items-center gap-2">
                      ...
                      <button
                        class="rounded-sm border border-gray-200 bg-white px-2 py-1 font-mono text-[10px] font-medium text-gray-600 transition-colors hover:bg-gray-100 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-300 dark:hover:bg-dark-700"
                        @click="handleCopyRequestId(row.request_id)"
                      >
                        {{ t('admin.ops.requestDetails.copy') }}
                      </button>
                    </div>
                    <span v-else class="font-mono text-[11px] text-gray-400">-</span>
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 text-right">
                    <button
                      v-if="row.kind === 'error' && row.error_id"
                      class="rounded-sm border border-red-200 bg-red-50 px-3 py-1.5 font-mono text-[11px] font-medium text-red-600 transition-colors hover:bg-red-100 dark:border-red-500/30 dark:bg-red-900/20 dark:text-red-300 dark:hover:bg-red-900/40"
                      @click="openErrorDetail(row.error_id)"
                    >
                      {{ t('admin.ops.requestDetails.viewError') }}
                    </button>
