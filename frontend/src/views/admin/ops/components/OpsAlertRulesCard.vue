...
<template>
  <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-dark-700 dark:bg-dark-900">
    <div class="mb-4 flex flex-wrap items-start justify-between gap-3 border-b border-dashed border-gray-200 pb-4 sm:gap-4 dark:border-dark-700">
      <div>
        <div class="eyebrow">{{ t('admin.ops.alertRules.title') }}</div>
    ...
    <div v-else-if="sortedRules.length === 0" class="rounded-md border border-dashed border-gray-200 p-8 text-center text-sm text-gray-500 dark:border-dark-700 dark:text-gray-400">
      {{ t('admin.ops.alertRules.empty') }}
    </div>

    <div v-else class="max-h-[520px] overflow-hidden rounded-md border border-gray-200 dark:border-dark-700">
      <div class="max-h-[520px] overflow-y-auto">
        ...
        <table v-else class="min-w-full divide-y divide-dashed divide-gray-200 dark:divide-dark-700">
          ...
          <tbody class="divide-y divide-dashed divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
            <tr v-for="row in sortedRules" :key="row.id" class="hover:bg-gray-50 dark:hover:bg-dark-800/60">
              <td class="px-4 py-3">
                <div class="text-xs font-medium text-gray-900 dark:text-white">{{ row.name }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-gray-500 dark:text-gray-400">
                  {{ row.description }}
                </div>
                <div v-if="row.updated_at" class="mt-1 font-mono text-[10px] tabular-nums text-gray-400">
                  {{ formatDateTime(row.updated_at) }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 font-mono text-xs tabular-nums text-gray-700 dark:text-gray-200">
                {{ row.metric_type }}
                <span class="mx-1 text-gray-400">{{ row.operator }}</span>
                {{ row.threshold }}
              </td>
              <td class="whitespace-nowrap px-4 py-3 font-mono text-xs font-medium text-gray-700 dark:text-gray-200">
                {{ row.severity }}
              </td>
    ...
    >
      <div class="space-y-4">
        <div v-if="!editorValidation.valid" class="rounded-md border border-red-200 bg-red-50 p-4 text-xs text-red-700 dark:border-red-900/50 dark:bg-red-900/20 dark:text-red-300">
          <div class="font-mono font-medium">{{ t('admin.ops.alertRules.validation.title') }}</div>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div class="flex items-center justify-between rounded-sm border border-gray-200 bg-gray-50 px-4 py-3 dark:border-dark-700 dark:bg-dark-800/60 md:col-span-2">
            <span class="font-mono text-xs font-medium text-gray-700 dark:text-gray-200">{{ t('admin.ops.alertRules.form.enabled') }}</span>
            <input v-model="draft!.enabled" type="checkbox" class="h-4 w-4 rounded-sm border-gray-300 text-primary-600 focus:ring-primary-500" />
          </div>

          <div class="flex items-center justify-between rounded-sm border border-gray-200 bg-gray-50 px-4 py-3 dark:border-dark-700 dark:bg-dark-800/60 md:col-span-2">
            <span class="font-mono text-xs font-medium text-gray-700 dark:text-gray-200">{{ t('admin.ops.alertRules.form.notifyEmail') }}</span>
            <input v-model="draft!.notify_email" type="checkbox" class="h-4 w-4 rounded-sm border-gray-300 text-primary-600 focus:ring-primary-500" />
          </div>
        </div>
      </div>
