  ...
  <BaseDialog :show="show" :title="title" width="full" :close-on-click-outside="true" @close="close">
    <div v-if="loading" class="flex items-center justify-center py-16">
      <div class="flex flex-col items-center gap-3">
        ...
        <div class="font-mono text-sm text-gray-500 dark:text-gray-400">{{ t('admin.ops.errorDetail.loading') }}</div>
      </div>
    </div>

    <div v-else-if="!detail" class="py-10 text-center font-mono text-sm text-gray-500 dark:text-gray-400">
      {{ emptyText }}
    </div>
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.requestId') }}</div>
          <div class="mt-1 break-all font-mono text-sm tabular-nums text-gray-900 dark:text-white">
            {{ requestId || '—' }}
          </div>
        </div>

        <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.time') }}</div>
        ...
        <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.platform') }}</div>
          <div class="mt-1 font-mono uppercase text-sm text-gray-900 dark:text-white">
            {{ detail.platform || '—' }}
          </div>
        </div>

        <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.group') }}</div>
          <div class="mt-1 text-sm text-gray-900 dark:text-white">
        ...
        <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.model') }}</div>
          <div class="mt-1 text-sm text-gray-900 dark:text-white">
        ...
        <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.inboundEndpoint') }}</div>
        ...
        <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.upstreamEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm text-gray-900 dark:text-white">
        ...
        <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.status') }}</div>
          <div class="mt-1">
            <span :class="['inline-flex items-center rounded-sm px-2 py-1 font-mono text-xs tabular-nums font-medium ring-1 ring-inset', statusClass]">
              {{ detail.status_code }}
            </span>
          </div>
        </div>
        ...
        <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.requestType') }}</div>
          <div class="mt-1 text-sm text-gray-900 dark:text-white">
        ...
        <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.message') }}</div>
          <div class="mt-1 truncate text-sm text-gray-900 dark:text-white" :title="detail.message">
            {{ detail.message || '—' }}
          </div>
        </div>

        <div v-if="detail.api_key_prefix" class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.apiKeyPrefix') }}</div>
          <div class="mt-1 font-mono text-sm tabular-nums text-gray-900 dark:text-white">
            {{ detail.api_key_prefix }}
          </div>
        </div>

        <div v-if="detail.attempted_key_prefix" class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.attemptedKeyPrefix') }}</div>
          <div class="mt-1 font-mono text-sm tabular-nums text-gray-900 dark:text-white">
            {{ detail.attempted_key_prefix }}
          </div>
        </div>

        <div v-if="detail.deleted_key_owner_email" class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/50">
          <div class="eyebrow">{{ t('admin.ops.errorDetail.deletedKeyOwner') }}</div>
          <div class="mt-1 text-sm text-gray-900 dark:text-white">
            ...
            <span class="ml-2 inline-flex items-center rounded-sm px-1.5 py-0.5 font-mono text-[10px] font-medium ring-1 ring-inset bg-red-50 text-red-700 ring-red-600/20 dark:bg-red-900/30 dark:text-red-400 dark:ring-red-500/30">
      ...
      <div class="rounded-md border border-gray-200 bg-gray-50 p-6 dark:border-dark-700 dark:bg-dark-800/50">
        <h3 class="eyebrow">{{ t('admin.ops.errorDetail.responseBody') }}</h3>
        <pre class="mt-4 max-h-[520px] overflow-auto rounded-sm border border-dark-800 bg-dark-950 p-4 font-mono text-xs text-gray-100"><code>{{ prettyJSON(primaryResponseBody || '') }}</code></pre>
      </div>

      <!-- Upstream errors list (only for request errors) -->
      <div v-if="showUpstreamList" class="rounded-md border border-gray-200 bg-gray-50 p-6 dark:border-dark-700 dark:bg-dark-800/50">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <h3 class="eyebrow">{{ t('admin.ops.errorDetails.upstreamErrors') }}</h3>
          <div class="font-mono text-xs text-gray-500 dark:text-gray-400" v-if="correlatedUpstreamLoading">{{ t('common.loading') }}</div>
        </div>

        <div v-if="!correlatedUpstreamLoading && !correlatedUpstreamErrors.length" class="mt-3 font-mono text-sm text-gray-500 dark:text-gray-400">
          {{ t('common.noData') }}
        </div>
        ...
        <div v-else class="mt-4 space-y-3">
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="font-mono text-xs tabular-nums text-gray-900 dark:text-white">
                #{{ idx + 1 }}
                <span v-if="ev.type" class="ml-2 rounded-sm bg-gray-100 px-2 py-0.5 font-mono text-[10px] font-medium text-gray-700 dark:bg-dark-700 dark:text-gray-200">{{ ev.type }}</span>
              </div>
              <div class="flex items-center gap-2">
                ...
                <button
                  type="button"
                  class="inline-flex items-center gap-1.5 rounded-sm border border-gray-200 bg-white px-1.5 py-1 font-mono text-[10px] font-medium text-gray-700 transition-colors hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-200 dark:hover:bg-dark-700"
            ...
            <div v-if="ev.message" class="mt-3 break-words text-sm text-gray-900 dark:text-white">{{ ev.message }}</div>

            <pre
              v-if="expandedUpstreamDetailIds.has(ev.id)"
              class="mt-3 max-h-[240px] overflow-auto rounded-sm border border-dark-800 bg-dark-950 p-3 font-mono text-xs text-gray-100"
