    ...
    <div class="space-y-6">
      ...
      <template v-else>
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 class="text-2xl font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.title') }}</h1>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div
            ...
            :key="item.key"
            class="rounded-md border border-gray-200 bg-white px-4 py-3 dark:border-dark-700 dark:bg-dark-900"
          >
            <div class="flex min-w-0 items-center gap-3">
              <div class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-sm" :class="item.iconClass">
                <Icon :name="item.icon" size="sm" />
              </div>
              <div class="min-w-0 flex-1">
                <div class="flex min-w-0 items-center justify-between gap-2">
                  <p class="eyebrow truncate">{{ item.label }}</p>
                  <span
                    v-if="item.badge"
                    class="inline-flex flex-shrink-0 items-center rounded-sm px-2 py-0.5 font-mono text-xs font-medium"
                ...
                <div class="mt-1 flex min-w-0 items-baseline gap-2">
                  <p class="truncate font-mono text-xl font-normal tabular-nums leading-7 tracking-tight text-gray-900 dark:text-white">{{ item.value }}</p>
        ...
        >
          <div data-test="pre-block-sync-card" class="card">
            <div class="flex flex-col gap-4 border-b border-dashed border-gray-200 px-6 py-4 dark:border-dark-700 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.preBlockSyncStatus') }}</h2>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.preBlockSyncHint') }}</p>
              </div>
              <span class="inline-flex w-fit items-center rounded-sm bg-gray-100 px-2.5 py-1 font-mono text-xs font-medium text-gray-600 dark:bg-dark-700 dark:text-gray-300">
            ...
            <div class="p-6">
              <div data-test="pre-block-metric-grid" class="grid grid-cols-2 gap-3 md:grid-cols-3">
                ...
                >
                  ...
                  <p class="mt-2 truncate font-mono text-2xl font-normal tabular-nums leading-8 tracking-tight" :class="item.valueClass">{{ item.value }}</p>
          ...
          <div data-test="pre-block-api-key-load-card" class="card">
            <div class="flex flex-col gap-4 border-b border-dashed border-gray-200 px-6 py-4 dark:border-dark-700 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.preBlockAPIKeyLoad') }}</h2>
            ...
            <div class="p-6">
                  ...
                  <div class="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                      <div class="flex min-w-0 items-center gap-2">
                        <span class="font-mono text-sm font-medium text-gray-900 dark:text-white">#{{ item.index + 1 }}</span>
                      ...
                      <p class="mt-1 font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">
                    ...
                    <div class="grid grid-cols-4 gap-2 text-right font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400 sm:min-w-[280px]">
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyActiveShort') }}</p>
                        <p class="mt-1 text-sm font-medium tabular-nums text-sky-700 dark:text-sky-300">{{ formatNumber(item.active) }}</p>
                      </div>
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyTotalShort') }}</p>
                        <p class="mt-1 text-sm font-medium tabular-nums text-gray-900 dark:text-white">{{ formatNumber(item.total) }}</p>
                      </div>
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyAvgShort') }}</p>
                        <p class="mt-1 text-sm font-medium tabular-nums text-gray-900 dark:text-white">{{ formatNumber(item.avg_latency_ms) }} ms</p>
                      </div>
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyLastShort') }}</p>
                        <p class="mt-1 text-sm font-medium tabular-nums text-gray-900 dark:text-white">{{ formatNumber(item.last_latency_ms) }} ms</p>
                      </div>
                    </div>
                  </div>
              ...
              <p v-else class="rounded-md border border-gray-200 bg-gray-50 p-4 text-sm text-gray-500 dark:border-dark-700 dark:bg-dark-800/60 dark:text-gray-400">
        ...
        <div v-if="showWorkerRuntimeCard" class="card">
          <div class="flex flex-col gap-4 border-b border-dashed border-gray-200 px-6 py-4 dark:border-dark-700 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 class="text-lg font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.workerStatus') }}</h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.workerStatusHint') }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-2 font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">
          ...
          <div class="grid grid-cols-1 gap-6 p-6 xl:grid-cols-[minmax(0,360px)_1fr]">
            <div class="space-y-4">
              <div class="rounded-md border border-gray-200 p-4 dark:border-dark-700">
                <div class="flex items-center justify-between gap-3">
                  ...
                  <span class="font-mono text-sm font-medium tabular-nums text-gray-900 dark:text-white">{{ queueUsagePercent }}</span>
                </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60">
                  <p class="eyebrow">{{ t('admin.riskControl.activeWorkers') }}</p>
                  <p class="mt-2 font-mono text-2xl font-normal tabular-nums tracking-tight text-gray-900 dark:text-white">{{ status?.active_workers ?? 0 }}</p>
                </div>
                <div class="rounded-md border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900/40 dark:bg-emerald-900/10">
                  <p class="eyebrow">{{ t('admin.riskControl.idleWorkers') }}</p>
                  <p class="mt-2 font-mono text-2xl font-normal tabular-nums tracking-tight text-emerald-700 dark:text-emerald-300">{{ status?.idle_workers ?? configForm.worker_count }}</p>
                </div>
                <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60">
                  <p class="eyebrow">{{ t('admin.riskControl.processed') }}</p>
                  <p class="mt-2 font-mono text-2xl font-normal tabular-nums tracking-tight text-gray-900 dark:text-white">{{ formatNumber(status?.processed ?? 0) }}</p>
                </div>
                <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/60">
                  <p class="eyebrow">{{ t('admin.riskControl.droppedErrors') }}</p>
                  <p class="mt-2 font-mono text-2xl font-normal tabular-nums tracking-tight text-gray-900 dark:text-white">{{ formatNumber((status?.dropped ?? 0) + (status?.errors ?? 0)) }}</p>
                </div>
              </div>
            </div>
            ...
            <div>
              <div class="mb-3 flex items-center justify-between gap-3">
                ...
                <span class="inline-flex items-center rounded-sm bg-gray-100 px-2.5 py-1 font-mono text-xs font-medium text-gray-600 dark:bg-dark-700 dark:text-gray-300">
                  {{ modeLabel(status?.mode ?? configForm.mode) }}
                </span>
              </div>
              <div class="grid grid-cols-2 gap-2 sm:grid-cols-4 md:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-10">
                <div
                  ...
                  :key="worker.id"
                  class="flex h-12 items-center justify-between rounded-sm border px-3 transition-colors"
                  :class="workerSlotClass(worker.state)"
                  :title="worker.label"
                >
                  <span class="font-mono text-sm font-medium tabular-nums">#{{ worker.id }}</span>
        ...
        <div class="card">
          <div class="flex flex-col gap-4 border-b border-dashed border-gray-200 px-6 py-4 dark:border-dark-700">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.records') }}</h2>
            ...
            <div class="flex flex-col gap-2 rounded-md border border-gray-200 bg-gray-50 px-3 py-2 dark:border-dark-700 dark:bg-dark-800/60 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div v-if="modelFilterPreviewModels.length > 0" class="flex flex-wrap gap-1.5">
                <span
                  ...
                  :key="model"
                  class="inline-flex max-w-[180px] items-center truncate rounded-sm border border-gray-200 bg-white px-2 py-1 font-mono text-xs text-gray-600 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-300"
                >
                  {{ model }}
                </span>
                <span v-if="hiddenModelFilterModelCount > 0" class="inline-flex rounded-sm border border-gray-200 bg-white px-2 py-1 font-mono text-xs text-gray-500 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-400">
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-700">
              <thead class="bg-gray-50 dark:bg-dark-800">
                <tr>
                  <th class="border-b border-dashed border-gray-200 px-5 py-3 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">{{ t('admin.riskControl.table.time') }}</th>
                  <th class="border-b border-dashed border-gray-200 px-5 py-3 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">{{ t('admin.riskControl.table.group') }}</th>
                  <th class="border-b border-dashed border-gray-200 px-5 py-3 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">{{ t('admin.riskControl.table.user') }}</th>
                  <th class="border-b border-dashed border-gray-200 px-5 py-3 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">{{ t('admin.riskControl.table.apiKey') }}</th>
                  <th class="border-b border-dashed border-gray-200 px-5 py-3 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">{{ t('admin.riskControl.table.endpoint') }}</th>
                  <th class="border-b border-dashed border-gray-200 px-5 py-3 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">{{ t('admin.riskControl.table.result') }}</th>
                  <th class="border-b border-dashed border-gray-200 px-5 py-3 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">{{ t('admin.riskControl.table.highest') }}</th>
                  <th class="border-b border-dashed border-gray-200 px-5 py-3 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">{{ t('admin.riskControl.table.actionMeta') }}</th>
                  <th class="border-b border-dashed border-gray-200 px-5 py-3 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">{{ t('admin.riskControl.table.latency') }}</th>
                  <th class="border-b border-dashed border-gray-200 px-5 py-3 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-dark-700 dark:text-gray-400">{{ t('admin.riskControl.table.input') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 bg-white dark:divide-dark-800 dark:bg-dark-900">
                ...
                <template v-else>
                  <tr v-for="row in logs" :key="row.id" class="hover:bg-gray-50 dark:hover:bg-dark-800/60">
                    <td class="whitespace-nowrap px-5 py-4 font-mono text-xs tabular-nums text-gray-700 dark:text-gray-300">{{ formatDateTime(row.created_at) }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-gray-700 dark:text-gray-300">{{ row.group_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                      <div>{{ row.user_email || '-' }}</div>
                      <div v-if="row.user_id" class="font-mono text-xs text-gray-400">UID {{ row.user_id }}</div>
                    </td>
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                      ...
                      <div class="font-mono text-xs text-gray-400">{{ row.provider || '-' }} / {{ row.model || '-' }}</div>
                    </td>
                    <td class="whitespace-nowrap px-5 py-4">
                      <span class="badge inline-flex items-center rounded-sm px-2 py-1 text-xs font-medium" :class="resultBadgeClass(row)">
                        {{ resultLabel(row) }}
                      </span>
                      </td>
                      <td class="whitespace-nowrap px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                        <div>{{ row.highest_category || '-' }}</div>
                      <div class="font-mono text-xs tabular-nums text-gray-400">{{ percent(row.highest_score) }}</div>
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                      ...
                      <div class="font-mono text-xs tabular-nums text-gray-400">
                      ...
                      <button
                        ...
                        type="button"
                        class="btn btn-sm mt-2 inline-flex items-center gap-1 border border-emerald-200 bg-emerald-50 px-2 py-1 text-xs font-medium text-emerald-700 hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-emerald-900/60 dark:bg-emerald-900/20 dark:text-emerald-300 dark:hover:bg-emerald-900/30"
                    ...
                    <td class="whitespace-nowrap px-5 py-4 font-mono text-xs tabular-nums text-gray-700 dark:text-gray-300">
                    ...
                    <td class="w-[320px] max-w-sm px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                      <button
                        type="button"
                        class="group flex w-full min-w-0 items-center gap-2 rounded-sm px-2 py-1.5 text-left transition-colors hover:bg-gray-100 dark:hover:bg-dark-800"
      ...
      <BaseDialog :show="settingsOpen" :title="t('admin.riskControl.settingsTitle')" width="extra-wide" @close="settingsOpen = false">
        <div class="space-y-6">
          <div class="flex gap-2 overflow-x-auto border-b border-dashed border-gray-200 pb-3 dark:border-dark-700">
            <button
              v-for="tab in settingsTabs"
              :key="tab.id"
              type="button"
              class="inline-flex whitespace-nowrap rounded-sm px-3 py-2 font-mono text-xs uppercase tracking-wide transition-colors"
              :class="activeSettingsTab === tab.id ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-dark-800 dark:hover:text-white'"
          ...
          <div v-if="activeSettingsTab === 'basic'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              <div class="flex items-center justify-between rounded-md border border-gray-200 p-4 dark:border-dark-700">
                <div>
            ...
            <div class="overflow-hidden rounded-md border border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-900">
              <div class="flex flex-col gap-4 border-b border-dashed border-gray-200 bg-gray-50 px-4 py-4 dark:border-dark-700 dark:bg-dark-800/60 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-start gap-3">
                  <span class="mt-0.5 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-sm bg-primary-50 text-primary-600 dark:bg-primary-900/30 dark:text-primary-300">
                    <Icon name="key" size="md" />
                  </span>
                  <div>
                    <label class="text-sm font-medium tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.apiKeys') }}</label>
              ...
              <div class="grid grid-cols-1 gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_minmax(360px,440px)]">
                <div class="space-y-3">
                  <div class="flex flex-col gap-2 rounded-md border border-gray-200 bg-gray-50 p-2 dark:border-dark-700 dark:bg-dark-800/60 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="inline-flex rounded-sm border border-gray-200 bg-white p-1 dark:border-dark-700 dark:bg-dark-800">
                      <button
                        type="button"
                        class="rounded-sm px-3 py-1.5 font-mono text-xs font-medium uppercase tracking-wide transition-colors"
                  ...
                  ></textarea>
                  <div class="flex flex-wrap items-center gap-2 font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">
                    <span class="inline-flex rounded-sm bg-gray-100 px-2 py-1 dark:bg-dark-700">
                      {{ t('admin.riskControl.inputApiKeyCount', { count: inputApiKeyCount }) }}
                    </span>
                    <span v-if="configForm.api_key_configured" class="inline-flex rounded-sm bg-gray-100 px-2 py-1 dark:bg-dark-700">
                      {{ t('admin.riskControl.storedApiKeyCount', { count: configForm.api_key_count }) }}
                    </span>
                    <span v-if="configForm.clear_api_key" class="inline-flex rounded-sm bg-red-50 px-2 py-1 text-red-700 dark:bg-red-900/20 dark:text-red-300">
                      {{ t('admin.riskControl.apiKeyWillClear') }}
                    </span>
                    <span v-else-if="pendingDeletedApiKeyCount > 0" class="inline-flex rounded-sm bg-amber-50 px-2 py-1 text-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
                      {{ t('admin.riskControl.apiKeyPendingDeleteCount', { count: pendingDeletedApiKeyCount }) }}
                    </span>
                    <span v-if="configForm.api_keys_mode === 'replace'" class="inline-flex rounded-sm bg-amber-50 px-2 py-1 text-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
                      {{ t('admin.riskControl.apiKeysReplaceWarning') }}
                    </span>
                  </div>

                  <div class="rounded-md border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800/60" @paste="handleModerationImagePaste">
                    <div class="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <p class="text-sm font-medium tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.auditTestInput') }}</p>
                      ...
                      <button
                        ...
                        type="button"
                        class="inline-flex items-center gap-1 rounded-sm px-2 py-1 font-mono text-xs font-medium text-gray-500 hover:bg-white hover:text-gray-900 dark:text-gray-400 dark:hover:bg-dark-800 dark:hover:text-white"
                    ...
                    ></textarea>
                    <div
                      class="mt-3 rounded-md border border-dashed border-gray-200 bg-white p-3 dark:border-dark-700 dark:bg-dark-800"
                      @dragover.prevent
                    ...
                    >
                      ...
                      <div v-if="moderationTestImages.length > 0" class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                        <div
                          ...
                          class="group relative aspect-square overflow-hidden rounded-sm border border-gray-200 bg-gray-100 dark:border-dark-700 dark:bg-dark-700"
                        >
                ...
                <div class="rounded-md border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800/60">
                  <div class="mb-3 flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <p class="text-sm font-medium tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.apiKeyHealth') }}</p>
                      <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.apiKeyFreezeRule') }}</p>
                    </div>
                    <span class="inline-flex shrink-0 items-center whitespace-nowrap rounded-sm border border-gray-200 bg-white px-2 py-0.5 font-mono text-[11px] font-medium leading-5 tabular-nums text-gray-600 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-300">
                      {{ t('admin.riskControl.apiKeyRows', { count: apiKeyRows.length }) }}
                    </span>
                  </div>

                  <div v-if="apiKeyRows.length === 0" class="flex min-h-32 flex-col items-center justify-center rounded-md border border-dashed border-gray-200 bg-white px-4 py-6 text-center dark:border-dark-700 dark:bg-dark-800">
                  ...
                  <div v-else class="space-y-2">
                    <div class="space-y-2" :class="apiKeyRowsExpanded ? 'max-h-72 overflow-y-auto pr-1' : ''">
                      <div
                        ...
                        class="rounded-md border bg-white p-2.5 dark:bg-dark-800"
                        :class="isStoredApiKeyPendingDelete(row) ? 'border-amber-200 opacity-70 dark:border-amber-800/60' : 'border-gray-200 dark:border-dark-700'"
                      >
                        <div class="flex items-start justify-between gap-2">
                          <div class="min-w-0">
                            <div class="flex min-w-0 flex-wrap items-center gap-2">
                              <span class="truncate font-mono text-sm font-medium text-gray-900 dark:text-white">{{ row.masked || '-' }}</span>
                              <span
                                class="badge inline-flex rounded-sm px-1.5 py-0.5 font-mono text-[11px] font-medium uppercase tracking-wide"
                            ...
                            <p class="mt-1 font-mono text-xs leading-5 tabular-nums text-gray-500 dark:text-gray-400">{{ apiKeyStatusMeta(row) }}</p>
                          </div>
                          <div class="flex flex-shrink-0 items-center gap-1.5">
                            <span class="badge inline-flex items-center gap-1.5 whitespace-nowrap rounded-sm px-2 py-0.5 text-xs font-medium" :class="apiKeyStatusBadgeClass(row.status)">
                            ...
                            <button
                              ...
                              type="button"
                              class="inline-flex h-7 w-7 items-center justify-center rounded-sm text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-dark-700 dark:hover:text-gray-200"
                        ...
                        <p v-if="row.last_error" class="mt-1.5 rounded-sm bg-amber-50 px-2 py-1.5 text-xs leading-5 text-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
                          {{ row.last_error }}
                        </p>
                      </div>
                    </div>

                    <div v-if="canToggleApiKeyRows" class="flex items-center justify-between gap-3 rounded-md border border-dashed border-gray-200 bg-white px-3 py-2 font-mono text-xs text-gray-500 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-400">
                      ...
                      <button
                        type="button"
                        class="btn btn-sm inline-flex shrink-0 items-center gap-1 rounded-sm px-2 py-1 font-medium text-primary-600 transition-colors hover:bg-primary-50 hover:text-primary-700 dark:text-primary-300 dark:hover:bg-primary-900/20"
                  ...
                  <div v-if="moderationTestResult" class="mt-4 rounded-md border border-gray-200 bg-white p-3 dark:border-dark-700 dark:bg-dark-800">
                    <div class="flex items-start justify-between gap-3">
                      <div>
                        <p class="text-sm font-medium tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.auditTestResult') }}</p>
                        <p class="mt-1 font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">
                      ...
                      <span class="badge inline-flex rounded-sm px-2 py-1 text-xs font-medium" :class="moderationTestResult.flagged ? 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300'">
                        {{ moderationTestResult.flagged ? t('admin.riskControl.auditTestFlagged') : t('admin.riskControl.auditTestPassed') }}
                      </span>
                    </div>
                    <div class="mt-3">
                      <div class="mb-2 flex items-center justify-between font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">
                        <span>{{ t('admin.riskControl.auditTestComposite') }}</span>
                        <span class="font-medium text-gray-900 dark:text-white">{{ percent(moderationTestResult.composite_score) }}</span>
                      </div>
                    ...
                    <div class="mt-3 max-h-52 space-y-2 overflow-y-auto pr-1">
                      <div v-for="score in moderationScoreRows" :key="score.category">
                        <div class="mb-1 flex items-center justify-between gap-3 font-mono text-xs tabular-nums">
                          <span class="truncate text-gray-600 dark:text-gray-300">{{ score.category }}</span>
                          <span class="text-gray-500 dark:text-gray-400">{{ percent(score.score) }} / {{ percent(score.threshold) }}</span>
                        </div>
          ...
          <div v-else-if="activeSettingsTab === 'scope'" class="space-y-5">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 class="text-base font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.groupScope') }}</h3>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.groupScopeHint') }}</p>
              </div>
              <div class="inline-flex rounded-sm border border-gray-200 bg-gray-100 p-1 dark:border-dark-700 dark:bg-dark-700">
                <button
                  type="button"
                  class="rounded-sm px-3 py-1.5 font-mono text-xs font-medium uppercase tracking-wide transition-colors"
            ...
            <div v-if="!configForm.all_groups" class="space-y-4">
              ...
              <div class="grid max-h-[420px] grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-3">
                <button
                  ...
                  :key="group.id"
                  type="button"
                  class="flex min-h-20 items-center justify-between rounded-md border p-4 text-left transition-colors"
                  :class="isGroupSelected(group.id) ? 'border-primary-300 bg-primary-50 dark:border-primary-700 dark:bg-primary-900/20' : 'border-gray-200 hover:bg-gray-50 dark:border-dark-700 dark:hover:bg-dark-700/60'"
                  @click="toggleGroup(group.id)"
                >
                  <span class="min-w-0">
                    <span class="block truncate text-sm font-medium tracking-tight text-gray-900 dark:text-white">{{ group.name }}</span>
                    <span class="mt-1 inline-flex rounded-sm bg-gray-100 px-2 py-0.5 font-mono text-xs text-gray-500 dark:bg-dark-700 dark:text-gray-400">{{ group.platform }}</span>
                  </span>
                  <span
            ...
            <div class="space-y-4 rounded-md border border-gray-200 p-4 dark:border-dark-700">
              <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <h3 class="text-base font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.modelFilter') }}</h3>
                  <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.modelFilterHint') }}</p>
                </div>
                <span class="inline-flex w-fit rounded-sm bg-gray-100 px-2.5 py-1 font-mono text-xs font-medium text-gray-600 dark:bg-dark-700 dark:text-gray-300">
                  {{ modelFilterSummary }}
                </span>
              </div>
              ...
              <div class="grid grid-cols-1 gap-2 md:grid-cols-3">
                ...
                >
                  <div class="flex items-center justify-between gap-2">
                    <span class="text-sm font-medium tracking-tight">{{ option.label }}</span>
                    <span
          ...
          <div v-else-if="activeSettingsTab === 'runtime'" class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="flex items-center justify-between rounded-md border border-gray-200 p-4 dark:border-dark-700 lg:col-span-2">
              <div>
            ...
            <div class="space-y-4 rounded-md border border-gray-200 p-4 dark:border-dark-700 lg:col-span-2">
          ...
          <div v-else-if="activeSettingsTab === 'riskThresholds'" class="space-y-5">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 class="text-base font-normal tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.riskThresholds') }}</h3>
            ...
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              ...
              >
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0">
                    <label class="block truncate font-mono text-sm font-medium tracking-tight text-gray-900 dark:text-white" :for="`risk-threshold-${row.category}`">
                      {{ row.category }}
                    </label>
                    <p class="mt-1 font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">
                  ...
                  <span class="inline-flex shrink-0 rounded-sm border border-gray-200 bg-white px-2 py-1 font-mono text-xs font-medium tabular-nums text-gray-600 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-300">
          ...
          <div v-else-if="activeSettingsTab === 'keywords'" class="space-y-5">
            ...
            >
              ...
              <div class="text-sm leading-6">
                <p class="font-medium tracking-tight" :class="keywordNotice.titleClass">{{ keywordNotice.title }}</p>
            ...
            <div>
              <div class="mb-2 flex items-center justify-between">
                ...
                <span class="inline-flex rounded-sm bg-gray-100 px-2 py-1 font-mono text-xs tabular-nums text-gray-500 dark:bg-dark-700 dark:text-gray-300">
              ...
              ></textarea>
              <p class="mt-2 font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">
          ...
          <div v-else class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="rounded-md border border-gray-200 p-4 text-sm text-gray-500 dark:border-dark-700 dark:text-gray-400 lg:col-span-2">
      ...
      >
        <div v-if="inputDetailRow" class="space-y-5">
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/70">
              <p class="eyebrow">{{ t('admin.riskControl.table.time') }}</p>
              <p class="mt-1 truncate font-mono text-xs tabular-nums text-gray-900 dark:text-white">{{ formatDateTime(inputDetailRow.created_at) }}</p>
            </div>
            <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/70">
              <p class="eyebrow">{{ t('admin.riskControl.table.user') }}</p>
              <p class="mt-1 truncate text-sm font-medium tracking-tight text-gray-900 dark:text-white">{{ inputDetailRow.user_email || '-' }}</p>
            </div>
            <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/70">
              <p class="eyebrow">{{ t('admin.riskControl.table.result') }}</p>
              <span class="badge mt-1 inline-flex rounded-sm px-2 py-1 text-xs font-medium" :class="resultBadgeClass(inputDetailRow)">
                {{ resultLabel(inputDetailRow) }}
              </span>
            </div>
            <div class="rounded-md border border-gray-200 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800/70">
              <p class="eyebrow">{{ t('admin.riskControl.table.highest') }}</p>
              <p class="mt-1 truncate font-mono text-xs tabular-nums text-gray-900 dark:text-white">
          ...
          <div class="rounded-md border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-800">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p class="text-sm font-medium tracking-tight text-gray-900 dark:text-white">{{ t('admin.riskControl.inputDetailContent') }}</p>
              ...
              <span class="badge inline-flex rounded-sm bg-sky-50 px-2.5 py-1 text-xs font-medium text-sky-700 dark:bg-sky-900/20 dark:text-sky-300">
                {{ inputDetailRow.group_name }}
              </span>
            </div>
            <pre class="mt-4 max-h-[420px] overflow-auto whitespace-pre-wrap break-words rounded-md bg-gray-950 p-4 font-mono text-sm leading-6 text-gray-100 dark:bg-black/50">{{ inputDetailText }}</pre>
          </div>
        </div>
...
function workerSlotClass(state: WorkerSlotState): string {
  ...
  return 'border-gray-200 bg-gray-50 text-gray-400 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-500'
}
