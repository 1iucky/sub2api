      ...
      <template #table>
        <DataTable :columns="columns" :data="monitors" :loading="loading">
          ...
          <template #cell-provider="{ row }">
            <span class="badge inline-flex items-center font-mono uppercase tracking-wide" :class="providerBadgeClass(row.provider)">
          ...
          <template #cell-availability_7d="{ row }">
            <span class="font-mono text-sm tabular-nums text-gray-900 dark:text-gray-100">{{ formatAvailability(row) }}</span>
          </template>

          <template #cell-latency="{ row }">
            <span class="font-mono text-sm tabular-nums text-gray-900 dark:text-gray-100">{{ formatLatency(row.primary_latency_ms) }}</span>
          </template>
