  ...
  <AppLayout>
      ...
      <template #table>
        ...
        >
          <template #cell-name="{ value }">
            <span class="font-medium tracking-tight text-gray-900 dark:text-white">{{ value }}</span>
          </template>
          ...
          <template #cell-group_count="{ row }">
            <span
              class="badge font-mono tabular-nums text-gray-700 dark:text-gray-300"
            >
              {{ (row.group_ids || []).length }}
              <span class="ml-0.5 font-normal tracking-wide text-gray-400 dark:text-dark-400">{{ t('admin.channels.groupsUnit', 'groups') }}</span>
            </span>
          </template>
          ...
          <template #cell-pricing_count="{ row }">
            ...
            >
              ...
              <span class="ml-0.5 font-normal tracking-wide text-gray-400 dark:text-dark-400">{{ t('admin.channels.pricingUnit', 'pricing rules') }}</span>
            </span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              >
                ...
                <span class="font-mono text-[10px] uppercase tracking-wide">{{ t('common.edit', 'Edit') }}</span>
              </button>
              <button
              ...
              >
                ...
                <span class="font-mono text-[10px] uppercase tracking-wide">{{ t('common.delete', 'Delete') }}</span>
              </button>
            </div>
          </template>
    ...
    >
      <div class="channel-dialog-body">
        <!-- Tab Bar -->
        <div class="flex items-center border-b border-dashed border-gray-200 dark:border-dark-700 flex-shrink-0 -mx-4 sm:-mx-6 px-4 sm:px-6 -mt-3 sm:-mt-4">
          <!-- Basic Settings Tab -->
          <button
            type="button"
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            ...
            <div class="divider-dashed -mx-1 pt-4">
          ...
          >
            <!-- Groups -->
            <div>
              <label class="eyebrow flex items-center gap-1">
                {{ t('admin.channels.form.groups', 'Associated Groups') }} <span class="text-red-500">*</span>
                <span v-if="section.group_ids.length > 0" class="ml-1 font-mono font-normal tabular-nums text-gray-400">
                  ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length }) }})
                </span>
              </label>
              <div class="max-h-40 overflow-auto rounded-md border border-gray-200 bg-gray-50 p-2 dark:border-dark-700 dark:bg-dark-900">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  <label
                    ...
                    :key="group.id"
                    class="inline-flex cursor-pointer items-center gap-1.5 rounded-sm border border-gray-200 px-2 py-1 text-xs transition-colors hover:bg-gray-100 dark:border-dark-700 dark:hover:bg-dark-800"
                    :class="[
                  ...
                  >
                    <input
                      ...
                      class="h-3 w-3 rounded-sm border-gray-300 text-primary-600 focus:ring-primary-500"
                      @change="toggleGroupInSection(sIdx, group.id)"
                    />
                    <span :class="['font-medium', platformTextClass(group.platform)]">{{ group.name }}</span>
                    <span
                      :class="['rounded-full px-1 py-0 font-mono tabular-nums text-[10px]', platformBadgeLightClass(group.platform)]"
                    >{{ group.rate_multiplier }}x</span>
                    <span class="font-mono tabular-nums text-[10px] text-gray-400">{{ group.account_count || 0 }}</span>
                    <span
            ...
            <div v-if="section.platform === 'anthropic' && webSearchGlobalEnabled" class="divider-dashed pt-3">
              <div class="flex items-center justify-between">
                <div>
            ...
            <div v-if="section.platform === 'openai'" class="divider-dashed pt-3">
            ...
            <div v-if="section.platform === 'anthropic'" class="divider-dashed pt-3">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <label class="text-xs font-medium text-gray-700 dark:text-gray-300">
            ...
            <div>
              <div class="mb-1 flex items-center justify-between">
                <label class="eyebrow">{{ t('admin.channels.form.modelMapping', 'Model Mapping') }}</label>
                <button type="button" @click="addMappingEntry(sIdx)" class="font-mono text-xs uppercase tracking-wide text-primary-600 hover:text-primary-700">
                  + {{ t('common.add', 'Add') }}
                </button>
              </div>
              <div
                v-if="Object.keys(section.model_mapping).length === 0"
                class="divider-dashed rounded-sm border border-dashed border-gray-300 p-2 text-center font-mono text-xs text-gray-400 dark:border-dark-600"
              >
              ...
              <div v-else class="space-y-1">
                ...
                >
                  ...
                  <button
                    ...
                    class="rounded-sm p-0.5 text-gray-400 hover:text-red-500"
                  >
                    <Icon name="trash" size="sm" />
                  </button>
                </div>
            ...
            <div>
              <div class="mb-1 flex items-center justify-between">
                <label class="eyebrow">{{ t('admin.channels.form.modelPricing', 'Model Pricing') }}</label>
                <div class="flex items-center gap-2">
                  <button
                    type="button"
                    @click="syncLatestModels(sIdx)"
                    :disabled="syncingPlatform === section.platform"
                    class="font-mono text-xs uppercase tracking-wide text-gray-500 hover:text-primary-600 disabled:opacity-50"
                  >
                    {{ syncingPlatform === section.platform ? t('admin.channels.form.syncingModels') : t('admin.channels.form.syncLatestModels') }}
                  </button>
                  <button type="button" @click="addPricingEntry(sIdx)" class="font-mono text-xs uppercase tracking-wide text-primary-600 hover:text-primary-700">
                    + {{ t('common.add', 'Add') }}
                  </button>
                </div>
              </div>
            ...
            <div class="divider-dashed mt-4 pt-4 space-y-3">
              <div class="flex items-center justify-between">
                <h4 class="text-sm font-medium tracking-tight text-gray-700 dark:text-gray-300">
                  {{ t('admin.channels.form.accountStatsPricingRules') }}
                </h4>
                <button
                  type="button"
                  @click="addAccountStatsRule(sIdx)"
                  class="rounded-sm border border-primary-300 px-3 py-1 font-mono text-xs font-medium uppercase tracking-wide text-primary-600 hover:bg-primary-50 dark:border-primary-700 dark:text-primary-400 dark:hover:bg-primary-900/20"
                >
              ...
              <p
                ...
                class="font-mono text-xs italic text-gray-400 dark:text-dark-400"
              >
              ...
              <div
                ...
                :key="ruleIndex"
                class="space-y-3 rounded-md border border-gray-200 p-4 dark:border-dark-700"
              >
                <div class="flex items-center justify-between">
                  <input
                    v-model="rule.name"
                    :placeholder="t('admin.channels.form.ruleName')"
                    class="bg-transparent text-sm font-medium tracking-tight text-gray-700 placeholder-gray-400 outline-none dark:text-gray-300"
                  />
                  <button type="button" @click="removeAccountStatsRule(sIdx, ruleIndex)" class="font-mono text-xs uppercase tracking-wide text-red-500 hover:text-red-700">
                    {{ t('common.delete') }}
                  </button>
                </div>

                <div>
                  <label class="eyebrow">{{ t('admin.channels.form.ruleGroups') }}</label>
                  <div class="mt-1 flex flex-wrap gap-1">
                    <label
                      ...
                      :class="rule.group_ids.includes(gid)
                        ...
                        : 'border-gray-200 hover:bg-gray-100 dark:border-dark-700 dark:hover:bg-dark-800'"
                    >
                      <input type="checkbox" :checked="rule.group_ids.includes(gid)" class="h-3 w-3 rounded-sm border-gray-300 text-primary-600 focus:ring-primary-500" @change="rule.group_ids.includes(gid) ? rule.group_ids.splice(rule.group_ids.indexOf(gid), 1) : rule.group_ids.push(gid)" />
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getGroupNameById(gid) }}</span>
                    </label>
                  </div>
                  <p v-if="section.group_ids.length === 0" class="mt-1 font-mono text-xs text-gray-400">
                    {{ t('admin.channels.form.noGroupsInChannel') }}
                  </p>
                </div>

                <div>
                  <label class="eyebrow">{{ t('admin.channels.form.ruleAccounts') }}</label>
                  <!-- Selected account chips -->
                  <div class="mt-1 flex flex-wrap gap-1">
                    <span
                      v-for="accountId in rule.account_ids"
                      :key="accountId"
                      class="inline-flex items-center gap-1 rounded-sm border border-primary-300 bg-primary-50 px-2 py-0.5 font-mono text-xs tabular-nums dark:border-primary-700 dark:bg-primary-900/20"
                    >
                  ...
                  <div class="relative mt-1 rule-account-search-container">
                    ...
                    <div
                      ...
                      class="absolute z-50 mt-1 max-h-48 w-full overflow-auto rounded-sm border border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-900"
                    >
                      <button
                      ...
                      >
                        ...
                        <span class="ml-2 font-mono text-xs tabular-nums text-gray-400">#{{ account.id }}</span>
                      </button>
                    </div>
                  </div>
                ...
                <div>
                  <div class="mb-1 flex items-center justify-between">
                    <label class="eyebrow">{{ t('admin.channels.form.ruleModelPricing') }}</label>
                    <button type="button" @click="addRulePricingEntry(sIdx, ruleIndex)" class="font-mono text-xs uppercase tracking-wide text-primary-600 hover:text-primary-700">
                      + {{ t('common.add') }}
                    </button>
                  </div>
                  <div v-if="rule.pricing.length === 0" class="divider-dashed rounded-sm border border-dashed border-gray-300 p-2 text-center font-mono text-xs text-gray-400 dark:border-dark-600">
...
.channel-tab-inactive {
  @apply border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-dark-400 dark:hover:text-gray-200;
}
</style>
