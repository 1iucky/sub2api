<template>
  <div class="flex min-h-screen items-center justify-center bg-gray-50 p-4 dark:bg-dark-950">
    <div
      class="w-full max-w-md space-y-4 rounded-lg border border-gray-200 bg-white p-6 dark:border-dark-700 dark:bg-dark-900"
    >
      <!-- Amount + Order ID -->
      <div v-if="amount" class="text-center">
        <p class="font-mono text-3xl tabular-nums" :style="{ color: methodColor }">¥{{ amount }}</p>
        <p v-if="orderId" class="mt-1 font-mono text-sm tabular-nums text-gray-500 dark:text-gray-400">
      ...
      <div v-if="error" class="space-y-3">
        <div
          class="rounded-sm border border-red-200 bg-red-50 p-3 text-sm text-red-600 dark:border-red-700 dark:bg-red-900/30 dark:text-red-400"
        >
          {{ error }}
        </div>
        <button
      ...
      <div v-else-if="success" class="space-y-3 py-4 text-center">
        <div class="text-5xl text-emerald-600 dark:text-emerald-400">✓</div>
