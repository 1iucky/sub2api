<template>
  <div class="flex min-h-screen items-center justify-center bg-gray-50 bg-blueprint px-4 dark:bg-dark-950">
    <div class="w-full max-w-md space-y-6">
      ...
      <template v-else>
        ...
        <div class="text-center">
          ...
          <p class="eyebrow mt-4">{{ statusEyebrow }}</p>
          <h2 class="mt-2 text-2xl font-normal tracking-tight text-gray-900 dark:text-white">
            {{ statusTitle }}
          </h2>
        ...
        <!-- Order Info -->
        <div v-if="order" class="rounded-lg border border-gray-200 bg-white p-5 dark:border-dark-700 dark:bg-dark-900">
          <div class="space-y-3 text-sm">
            ...
            <div v-if="hasAmountFields(order)" class="divider-dashed flex justify-between pt-3">
              <span class="font-medium text-gray-700 dark:text-gray-300">{{ t('payment.orders.payAmount') }}</span>
              <span class="font-mono tabular-nums font-medium text-primary-600 dark:text-primary-400">{{ formatGatewayAmount(order.pay_amount) }}</span>
            </div>
        ...
        <div v-else-if="returnInfo" class="rounded-lg border border-gray-200 bg-white p-5 dark:border-dark-700 dark:bg-dark-900">
          <div class="space-y-3 text-sm">
...
const statusEyebrow = computed(() => {
  if (isSuccess.value) {
    return String(t('common.success') || 'SUCCESS').toUpperCase()
  }
  if (isPending.value) {
    return 'PENDING'
  }
  return String(t('common.error') || 'ERROR').toUpperCase()
})
