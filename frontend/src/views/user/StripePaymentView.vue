  ...
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-gray-50 dark:bg-dark-900' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        ...
        <h3 class="text-lg font-normal tracking-tight text-gray-900 dark:text-white">{{ t('payment.stripeLoadFailed') }}</h3>
      ...
      <template v-else>
        ...
        <div v-if="order" class="card overflow-hidden">
          <div class="border-b border-gray-200 bg-white px-6 py-6 text-center dark:border-dark-700 dark:bg-dark-900">
            <p class="eyebrow">{{ t('payment.actualPay') }}</p>
            <p class="mt-2 font-mono text-3xl tabular-nums text-gray-900 dark:text-white">{{ formatGatewayAmount(order.pay_amount) }}</p>
          </div>
        </div>
        ...
        <template v-if="wechatQrUrl">
          <div class="card p-6">
            <div class="flex flex-col items-center space-y-4">
              <p class="eyebrow">{{ t('payment.qr.scanWxpay') }}</p>
              <div class="relative rounded-md border border-[#2BB741] bg-white p-4 dark:border-[#2BB741]/70 dark:bg-dark-800">
                <img :src="wechatQrUrl" alt="WeChat Pay QR" class="h-56 w-56 rounded-sm" />
                <div class="pointer-events-none absolute inset-0 flex items-center justify-center">
                  <span class="rounded-full bg-[#2BB741] p-2 ring-2 ring-white dark:ring-dark-900">
        ...
        <template v-else-if="stripeSuccess">
          <div class="card p-6 text-center">
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <p class="eyebrow">{{ t('payment.result.success') }}</p>
        ...
        <template v-else-if="showPaymentElement">
          <div class="card p-6">
            ...
            <button class="btn btn-stripe rounded-md mt-6 w-full py-3 text-base" :disabled="stripeSubmitting || !stripeReady" @click="handleGenericPay">
