<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div v-if="!platform" class="flex items-start gap-3 p-3 rounded-md bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
      ...
      <template v-else>
        ...
        <div v-if="clientTabs.length" class="overflow-x-auto border-b border-gray-200 dark:border-dark-700">
          <nav class="-mb-px flex min-w-max gap-4 sm:gap-6" aria-label="Client">
            <button
              ...
              :class="[
                'whitespace-nowrap py-2.5 px-1 border-b-2 font-mono uppercase tracking-wide text-[11px] transition-colors duration-150',
        ...
        <div class="space-y-4">
          ...
          >
            <!-- File Hint (if exists) -->
            <p v-if="file.hint" class="text-xs text-amber-600 dark:text-amber-400 mb-1.5 flex items-center gap-1 font-mono">
              <Icon name="exclamationCircle" size="sm" class="flex-shrink-0" />
              {{ file.hint }}
            </p>
            <div class="bg-gray-900 dark:bg-dark-950 rounded-md border border-gray-700 dark:border-dark-700 overflow-hidden">
              <!-- Code Header -->
              <div class="flex items-center justify-between gap-3 px-3 py-2 bg-gray-900 dark:bg-dark-900 border-b border-gray-700 dark:border-dark-700">
                <span class="min-w-0 truncate text-xs text-gray-400 dark:text-gray-500 font-mono">{{ file.path }}</span>
                <button
                  type="button"
                  @click="copyContent(file.content, index)"
                  class="flex flex-shrink-0 items-center gap-1.5 px-2 py-1 text-[11px] font-mono uppercase tracking-wide rounded-sm border transition-colors duration-150"
                  :class="copiedIndex === index
                    ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-400'
                    : 'border-gray-600 dark:border-dark-600 text-gray-400 hover:text-primary-400 hover:border-primary-500/50'"
                >
                  <svg v-if="copiedIndex === index" class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                  <svg v-else class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
              ...
              <!-- Code Content -->
              <pre class="p-4 text-[13px] leading-relaxed font-mono text-gray-100 dark:text-gray-200 overflow-x-auto"><code v-if="file.highlighted" v-html="file.highlighted"></code><code v-else v-text="file.content"></code></pre>
            </div>
          </div>
        </div>

        <!-- Usage Note -->
        <div v-if="showPlatformNote" class="flex items-start gap-3 p-3 rounded-md bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
...
const keyword = (value: string) => wrapToken('text-primary-400', value)
const variable = (value: string) => wrapToken('text-sky-300', value)
const operator = (value: string) => wrapToken('text-gray-500', value)
const string = (value: string) => wrapToken('text-amber-300', value)
const comment = (value: string) => wrapToken('text-gray-600', value)
