...
<template>
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    <div
      ...
      :key="index"
      class="flex items-center gap-1.5 rounded-sm border border-gray-200 bg-white px-2.5 py-1.5 text-xs transition-colors duration-150 hover:border-primary-300 dark:border-dark-700 dark:bg-dark-900 dark:hover:border-primary-700"
    >
      <span class="font-mono uppercase tracking-wide text-[10px] text-gray-600 dark:text-gray-400">{{ item.name }}</span>
      <span
        v-if="item.isDefault"
        class="rounded-sm border border-primary-200 bg-primary-50 px-1 py-px text-[10px] font-medium leading-tight text-primary-600 dark:border-primary-800/60 dark:bg-primary-900/30 dark:text-primary-400"
      ...
      <div class="group/endpoint relative flex items-center gap-1.5">
        <div
          class="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max max-w-[24rem] -translate-x-1/2 translate-y-1 rounded-sm border border-gray-200 bg-white px-3 py-2 text-left opacity-0 transition-all duration-150 group-hover/endpoint:translate-y-0 group-hover/endpoint:opacity-100 group-focus-within/endpoint:translate-y-0 group-focus-within/endpoint:opacity-100 dark:border-dark-700 dark:bg-dark-900"
        >
          <p
            v-if="item.description"
            class="max-w-[24rem] break-words text-xs leading-5 text-gray-600 dark:text-gray-300"
          >
            {{ item.description }}
          </p>
          <p
            class="flex items-center gap-1.5 text-[11px] leading-4 text-primary-600 dark:text-primary-400"
        ...
        <button
          type="button"
          class="rounded-sm p-0.5 transition-colors duration-150"
        ...
        <a
          ...
          class="rounded-sm p-0.5 text-gray-400 transition-colors duration-150 hover:text-amber-500 dark:text-gray-500 dark:hover:text-amber-400"
