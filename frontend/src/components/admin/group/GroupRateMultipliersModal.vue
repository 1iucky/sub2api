<template>
  <BaseDialog :show="show" :title="t('admin.groups.rateMultipliersTitle')" width="wide" @close="handleClose">
    <div v-if="group" class="space-y-4">
      <!-- 分组信息 -->
      <div class="flex flex-wrap items-center gap-3 rounded-md border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm dark:border-dark-700 dark:bg-dark-900">
      ...
      <div class="rounded-md border border-gray-200 p-3 dark:border-dark-700">
        ...
        <div class="flex items-end gap-2">
          <div class="relative flex-1">
            ...
            <div
              ...
              class="absolute left-0 right-0 top-full z-10 mt-1 max-h-48 overflow-y-auto rounded-sm border border-gray-200 bg-white dark:border-dark-600 dark:bg-dark-800"
            >
              <button
                v-for="user in searchResults"
                :key="user.id"
                type="button"
                class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm hover:bg-gray-50 dark:hover:bg-dark-700"
                @click="selectUser(user)"
              >
                <span class="font-mono tabular-nums text-gray-400">#{{ user.id }}</span>
        ...
        <!-- 批量调整 + 全部清空 -->
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center gap-3 border-t border-dashed border-gray-200 pt-3 dark:border-dark-700">
          ...
          <div class="flex items-center gap-1.5">
            ...
            <input
              ...
              min="0"
              autocomplete="off"
              class="hide-spinner w-20 rounded-sm border border-gray-300 bg-white px-2 py-1 text-center font-mono text-sm tabular-nums transition-colors focus:border-primary-500 focus:outline-none dark:border-dark-700 dark:bg-dark-900 dark:focus:border-primary-500"
              placeholder="0.5"
            />
            <button
              type="button"
          ...
          <div class="ml-auto">
            <button
              type="button"
              class="rounded-sm border border-red-200 bg-red-50 px-3 py-1.5 text-sm font-medium text-red-600 transition-colors hover:bg-red-100 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40"
              @click="clearAllLocal"
            >
            ...
            <div class="max-h-[420px] overflow-y-auto">
              <table class="w-full text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-gray-200 bg-gray-50 dark:border-dark-700 dark:bg-dark-900">
                    <th class="px-3 py-2 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userEmail') }}</th>
                    <th class="px-3 py-2 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">ID</th>
                    <th class="px-3 py-2 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userName') }}</th>
                    <th class="px-3 py-2 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userNotes') }}</th>
                    <th class="px-3 py-2 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.userStatus') }}</th>
                    <th class="px-3 py-2 text-left font-mono text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ t('admin.groups.columns.rateMultiplier') }}</th>
                    <th v-if="showFinalRate" class="px-3 py-2 text-left font-mono text-xs font-medium uppercase tracking-wide text-primary-600 dark:text-primary-400">{{ t('admin.groups.finalRate') }}</th>
                ...
                <tbody class="divide-y divide-dashed divide-gray-200 dark:divide-dark-700">
                  ...
                  >
                    ...
                    <td class="whitespace-nowrap px-3 py-2 font-mono tabular-nums text-gray-400 dark:text-gray-500">{{ entry.user_id }}</td>
                    ...
                    <td v-if="showFinalRate" class="whitespace-nowrap px-3 py-2 font-mono font-medium tabular-nums text-primary-600 dark:text-primary-400">
      ...
      <!-- 底部操作栏 -->
      <div class="flex items-center gap-3 border-t border-dashed border-gray-200 pt-4 dark:border-dark-700">
        <!-- 左侧：未保存提示 + 撤销 -->
