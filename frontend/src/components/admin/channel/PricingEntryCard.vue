<template>
  <div class="rounded-md border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-900">
      ...
      <div v-if="collapsed" class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        ...
        <div class="flex min-w-0 flex-1 flex-wrap items-center gap-1">
          ...
          <span
            ...
            class="whitespace-nowrap font-mono tabular-nums text-xs text-gray-400"
          >
        ...
        <!-- Billing mode badge -->
        <span
          class="flex-shrink-0 rounded-sm bg-primary-100 px-2 py-0.5 font-mono text-xs font-medium text-primary-700 dark:bg-primary-900/30 dark:text-primary-300"
        >
          {{ billingModeLabel }}
        </span>
      </div>
      ...
      <div class="collapsible-inner">
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <div class="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-6">
            <div>
              <label class="font-mono text-xs text-gray-400">{{ t('admin.channels.form.inputPrice') }}</label>
              <input :value="entry.input_price" @input="emitField('input_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 font-mono text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="font-mono text-xs text-gray-400">{{ t('admin.channels.form.outputPrice') }}</label>
              <input :value="entry.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 font-mono text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="font-mono text-xs text-gray-400">{{ t('admin.channels.form.cacheWritePrice') }}</label>
              <input :value="entry.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 font-mono text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="font-mono text-xs text-gray-400">{{ t('admin.channels.form.cacheReadPrice') }}</label>
              <input :value="entry.cache_read_price" @input="emitField('cache_read_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 font-mono text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="font-mono text-xs text-gray-400">{{ t('admin.channels.form.imageInputPrice') }}</label>
              <input :value="entry.image_input_price" @input="emitField('image_input_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 font-mono text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
            </div>
            <div>
              <label class="font-mono text-xs text-gray-400">{{ t('admin.channels.form.imageTokenPrice') }}</label>
          ...
          <div v-if="!hideTokenIntervals" class="mt-3">
            <div class="flex items-center justify-between">
              ...
              <button type="button" @click="addInterval" class="font-mono text-xs text-primary-600 hover:text-primary-700">
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <div class="mt-1 w-48">
            <input :value="entry.per_request_price" @input="emitField('per_request_price', ($event.target as HTMLInputElement).value)"
              type="number" step="any" min="0" class="input font-mono text-sm tabular-nums" :placeholder="t('admin.channels.form.pricePlaceholder')" />
          </div>
          ...
          <div v-else class="mt-2 rounded-sm border border-dashed border-gray-300 p-3 text-center font-mono text-xs text-gray-400 dark:border-dark-600">
        ...
        <div v-else-if="entry.billing_mode === 'image' || entry.billing_mode === 'video'">
          ...
          <div class="mt-3 flex items-center justify-between">
            ...
            <button type="button" @click="addMediaTier" class="font-mono text-xs text-primary-600 hover:text-primary-700">
              + {{ t('admin.channels.form.addTier') }}
            </button>
          </div>
          <div v-if="entry.intervals && entry.intervals.length > 0" class="mt-2 space-y-2">
