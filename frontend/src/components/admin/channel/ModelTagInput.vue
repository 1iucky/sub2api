<template>
  <div>
    <!-- Tags display -->
    <div class="flex flex-wrap gap-1.5 rounded-md border border-gray-200 bg-white p-2 dark:border-dark-700 dark:bg-dark-900 min-h-[2.5rem]">
      <span
        v-for="(model, idx) in models"
        :key="idx"
        class="inline-flex items-center gap-1 rounded-sm px-2 py-0.5 font-mono text-sm"
      ...
      <input
        ...
        v-model="inputValue"
        type="text"
        class="flex-1 min-w-[120px] border-none bg-transparent font-mono text-sm outline-none placeholder:text-gray-400 dark:text-white"
