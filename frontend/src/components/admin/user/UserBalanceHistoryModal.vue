  ...
  <BaseDialog :show="show" :title="t('admin.users.balanceHistoryTitle')" width="wide" :close-on-click-outside="true" :z-index="40" @close="$emit('close')">
    <div v-if="user" class="space-y-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              ...
              <span v-if="user.deleted_at" class="flex-shrink-0 inline-flex items-center rounded-sm px-1 py-px font-mono text-[10px] font-medium leading-tight bg-rose-100 text-rose-600 ring-1 ring-inset ring-rose-200 dark:bg-rose-500/20 dark:text-rose-400 dark:ring-rose-500/30">
                {{ t('admin.usage.userDeletedBadge') }}
              </span>
              <span
                v-if="user.username"
                class="flex-shrink-0 rounded-sm bg-primary-50 px-1.5 py-0.5 font-mono text-xs text-primary-600 dark:bg-primary-900/20 dark:text-primary-400"
              >
                {{ user.username }}
              </span>
            </div>
            <p class="font-mono text-xs text-gray-400 dark:text-dark-500">
              {{ t('admin.users.createdAt') }}: {{ formatDateTime(user.created_at) }}
            </p>
          </div>
          <!-- Current balance: prominent display on the right -->
          <div class="flex-shrink-0 text-right">
            <p class="eyebrow !text-gray-500 dark:!text-dark-400">{{ t('admin.users.currentBalance') }}</p>
            <p class="font-mono text-xl font-normal tabular-nums text-primary-600 dark:text-primary-400">
              ${{ user.balance?.toFixed(2) || '0.00' }}
            </p>
          </div>
        </div>
        <!-- Row 2: notes + total recharged -->
        <div class="mt-2.5 flex items-center justify-between border-t border-dashed border-gray-200 pt-2.5 dark:border-dark-600">
          ...
          <p class="ml-4 flex-shrink-0 text-xs text-gray-500 dark:text-dark-400">
            {{ t('admin.users.totalRecharged') }}: <span class="font-mono font-medium tabular-nums text-emerald-600 dark:text-emerald-400">${{ totalRecharged.toFixed(2) }}</span>
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        >
          <Icon name="plus" size="sm" class="mr-1 text-emerald-500" :stroke-width="2" />
        ...
        <button
          ...
          class="btn btn-secondary"
        >
          <svg class="mr-1 h-4 w-4 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
