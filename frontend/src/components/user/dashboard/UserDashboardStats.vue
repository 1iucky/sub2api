<template>
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    ...
    <div v-if="!isSimple" class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="eyebrow">{{ t('dashboard.balance') }}</p>
          <p class="font-mono text-xl font-normal tabular-nums tracking-tight text-emerald-600 dark:text-emerald-400">${{ formatBalance(balance) }}</p>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-md bg-gray-100 p-2 dark:bg-dark-800 dark:bg-dark-700/40">
          <Icon name="key" size="md" class="text-gray-700 dark:text-gray-300" :stroke-width="2" />
        </div>
        <div>
          <p class="eyebrow">{{ t('dashboard.apiKeys') }}</p>
          <p class="font-mono text-xl font-normal tabular-nums tracking-tight text-gray-900 dark:text-white">{{ stats?.total_api_keys || 0 }}</p>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="eyebrow">{{ t('dashboard.todayRequests') }}</p>
          <p class="font-mono text-xl font-normal tabular-nums tracking-tight text-gray-900 dark:text-white">{{ stats?.today_requests || 0 }}</p>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="eyebrow">{{ t('dashboard.todayCost') }}</p>
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="eyebrow">{{ t('dashboard.todayTokens') }}</p>
          <p class="font-mono text-xl font-normal tabular-nums tracking-tight text-gray-900 dark:text-white">{{ formatTokens(stats?.today_tokens || 0) }}</p>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="eyebrow">{{ t('dashboard.totalTokens') }}</p>
          <p class="font-mono text-xl font-normal tabular-nums tracking-tight text-gray-900 dark:text-white">{{ formatTokens(stats?.total_tokens || 0) }}</p>
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          ...
          <div class="flex items-baseline gap-2">
            <p class="font-mono text-xl font-normal tabular-nums tracking-tight text-gray-900 dark:text-white">{{ formatTokens(stats?.rpm || 0) }}</p>
            <span class="font-mono text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">RPM</span>
          </div>
          <div class="flex items-baseline gap-2">
            <p class="font-mono text-sm font-normal tabular-nums tracking-tight text-primary-600 dark:text-primary-400">{{ formatTokens(stats?.tpm || 0) }}</p>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="eyebrow">{{ t('dashboard.avgResponse') }}</p>
          <p class="font-mono text-xl font-normal tabular-nums tracking-tight text-gray-900 dark:text-white">{{ formatDuration(stats?.average_duration_ms || 0) }}</p>
  ...
  <div v-if="!isSimple && platformCards.length > 0" class="card p-4">
    <div class="mb-3 flex items-center justify-between">
      <h3 class="eyebrow !text-sm !normal-case !tracking-wide text-gray-700 dark:text-gray-300">{{ t('dashboard.platformBreakdown') }}</h3>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <div
        ...
        :class="[
          ...
          item.isOther
            ? 'border-dashed border-gray-300 bg-gray-50 dark:border-dark-600 dark:bg-dark-800/40'
            : 'border-gray-200 dark:border-dark-700'
        ]"
      >
        <div class="flex items-center justify-between">
          <span class="text-sm font-normal text-gray-900 dark:text-white">
            {{ item.isOther ? t('dashboard.platformOther') : platformLabel(item.platform) }}
          </span>
          <span class="font-mono text-sm tabular-nums text-primary-600 dark:text-primary-400" :title="t('dashboard.actual')">
            ${{ formatCost(item.total_actual_cost) }}
          </span>
        </div>
        <div class="mt-2 space-y-1 text-xs">
          <div class="flex items-center justify-between border-b border-dashed border-gray-200 pb-1 dark:border-dark-700">
        ...
        <div v-if="hasAnyLimit(item.quota) && !item.isOther" class="mt-3 space-y-1.5 border-t border-dashed border-gray-200 pt-2 dark:border-dark-700">
          <p class="eyebrow !text-[10px]">
