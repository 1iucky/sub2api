<template>
  <div class="group/usage relative text-sm">
    <div class="flex items-center gap-1.5">
      ...
      <span class="font-mono font-medium tabular-nums text-gray-900 dark:text-white">${{ today.toFixed(4) }}</span>
      <Icon
        v-if="hasBreakdown"
        name="infoCircle"
        size="xs"
    ...
    <div class="mt-0.5 flex items-center gap-1.5">
      ...
      <span class="font-mono font-medium tabular-nums text-gray-900 dark:text-white">${{ total.toFixed(4) }}</span>
    </div>

    <div
      v-if="hasBreakdown"
      class="pointer-events-none absolute left-full top-0 z-50 ml-2 min-w-[220px] whitespace-nowrap rounded-sm border border-gray-200 bg-white px-3 py-2 text-xs text-gray-700 opacity-0 transition-opacity duration-100 group-hover/usage:opacity-100 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-200"
    >
      <div class="mb-1.5 flex items-center justify-between gap-3 border-b border-dashed border-gray-200 pb-1 text-[11px] uppercase tracking-wide opacity-80 dark:border-dark-700">
        <span class="font-mono">{{ t('admin.users.platformBreakdown') }}</span>
